...
def cargar_todo():
    ...
    stats_genero = sc.calcular_estadisticas_genero(df)
    return df, modelo, origen, rangos_genero, rango_ambiguo, palabras_genero, stats_genero


df_canciones, modelo, origen_modelo, RANGOS_GENERO, RANGO_AMBIGUO, PALABRAS_GENERO, STATS_GENERO = cargar_todo()
...
if "messages" not in st.session_state:
    st.session_state.messages = [{
        ...
        "content": "👋 **¡Hola! Soy Síncopa**, tu asistente coreográfico.\n\n"
                   ...
                   "🔗 **Pégame el link de la canción** desde **Spotify, YouTube o Apple Music**.\n\n"
                   "⚠️ **Cómo trabajo:**\n"
...
def analizar_entrada(entrada):
    ...
    if not entrada.startswith("http"):
        return {"tipo": "no_es_link", "titulo": entrada}
...
with tabs[0]:
    ...
    if prompt := st.chat_input("Pega el link de Spotify/YouTube/Apple Music, o pide sugerencias, vestuario o entrenamiento..."):
        ...
        with st.chat_message("assistant"):
            ...
            if genero_mencionado and pide_sugerencia:
                ...
                if tipo == "no_es_link":
                    reply = ("🔗 **Necesito un link, no un texto**\n\n"
                              "Para evaluar una canción, pégame el **link directo** desde Spotify, YouTube o "
                              "Apple Music. No proceso títulos escritos a mano.")

                elif tipo == "plataforma_no_soportada":
                    ...
                    par, grp, sol, metrica, _ = sc.DETALLES_COREOGRAFICOS[genero_catalogo]
                    ...
                    aprovechamiento = sc.comentar_aprovechamiento(
                        genero_catalogo, genero_modelo, resultado["features"], STATS_GENERO
                    )
