...
def analizar_audio_primeros_30s(url):
  ...
  try:
    ...
    # =========================================================================
    # ANÁLISIS ACÚSTICO PURO (Sin listas de palabras, basado en física del sonido)
    # =========================================================================
    ...
    # Medición de periodicidad del ritmo (fuerza y regularidad de los beats)
    # La voz hablada o los videos de charla tienen una regularidad rítmica muy baja o errática.
    # Evaluamos la variabilidad de los intervalos entre beats (pulse clarity / strength)
    spec_flatness = np.mean(librosa.feature.spectral_flatness(y=y))
    zcr = np.mean(librosa.feature.zero_crossing_rate(y=y))

    # Si hay muy pocos beats detectados o la señal es predominantemente plana/hablada (alta planitud espectral y zcr irregular)
    # se cataloga como no musical puramente por acústica.
